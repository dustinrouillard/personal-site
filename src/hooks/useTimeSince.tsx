...
function FormatTimeSince(date: Date): string {
  ...
  const pluralize = (unit: string, value: number) => `${value} ${unit}${value !== 1 ? 's' : ''}`;
  ...
  switch (true) {
    ...
    case diffInSeconds < 12 * 30 * 24 * 60 * 60:
      return pluralize('month', Math.floor(diffInSeconds / (30 * 24 * 60 * 60)));
    default:
      return pluralize('year', Math.floor(diffInSeconds / (365 * 24 * 60 * 60)));
  }
}
